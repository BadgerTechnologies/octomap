...
int main(int argc, char** argv) {
  ...
  // ------------------------------------------------------------
    ...
    // Floating point math varies somewhat by architecture
    // Do not expect an exact size, but one that is close to nominal.
    EXPECT_NEAR ((double)tree.size(), 50487.0 + 8 * tree.getTreeDepth(), 20.0);
  ...
  // write graph test
    ...
    // Floating point math varies somewhat by architecture
    // Do not expect an exact size, but one that is close to nominal.
    EXPECT_NEAR ((double)tree.size(), 53831.0 + 8 * tree.getTreeDepth(), 20.0);
